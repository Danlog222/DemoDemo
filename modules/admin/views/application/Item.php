...
<div class="card" style="width: 18rem;">
  ...
  <li class="list-group-item">Номер заявки: <?=Html::encode($model->id)?></li>
    ...
    <li class="list-group-item">Логин пользователя: <?=Html::encode(User::findOne($model->user_id)->login)?></li>
  </ul>
  <div class="card-body">
  <?= Html::a('Просмотр', ['view', 'id' => $model->id], ['class' => 'btn btn-primary ']) ?>
  <?= $model->status_id == 1 
   ?Html::a('Оказать услугу', ['done', 'id' => $model->id], ['class' => 'btn btn-success m-1']) :''?>
   <?= $model->status_id == 1 
   ?Html::a('Отклонить', ['cancel', 'id' => $model->id], ['class' => 'btn btn-danger m-1']) :''?>
  </div>
</div>
